...
/*
   S17FS * fs_format(const char *const fname);
   1   Normal
   2   NULL
   3   Empty string
   S17FS *fs_mount(const char *const fname);
   1   Normal
   2   NULL
   3   Empty string
   int fs_unmount(S17FS *fs);
   1   Normal
   2   NULL
   */
///*
TEST(a_tests, format_mount_unmount) {
    const char *test_fname = "a_tests.S17FS";
    S17FS *fs = NULL;
    // FORMAT 2
...
/*
   ...
   */
/*TEST(b_tests, file_creation_zero_01) {
  const char *test_fname = "b_tests_normal.S17FS";
  S17FS *fs = fs_format(test_fname);
  ASSERT_NE(fs, nullptr);
...
TEST(b_tests, file_creation_zero_02) {
const char *test_fname = "b_tests_normal.S17FS";
S17FS *fs = fs_format(test_fname);
ASSERT_NE(fs, nullptr);
...
TEST(b_tests, file_creation_zero_03) {
const char *test_fname = "b_tests_normal.S17FS";
S17FS *fs = fs_format(test_fname);
ASSERT_NE(fs, nullptr);

// CREATE_FILE 1
ASSERT_EQ(fs_create(fs, "/folder/with_file", FS_REGULAR), 0);
score += 1;
}
*/
///*
...
TEST(b_tests, file_creation_two) {
    ...
    const char *test_fname = "b_tests_full_table.S17FS";
    S17FS *fs            = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
    ...
    //>>>>>>> 70e154015c23f06cbda8d3c0b68bcec4fa66f62a
...
//*/
/*
   int fs_open(S17FS *fs, const char *path)
    ...
    vector<const char *> filenames{
        "/file", "/folder", "/folder/with_file", "/folder/with_folder", "/DOESNOTEXIST", "/file/BAD_REQUEST",
            ...
            "/folder/withwaytoolongfilenamethattakesupmorespacethanitshould and yet was not enough so I had to add "
                "more/bad_req",
            "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
    const char *test_fname = "c_tests.S17FS";
    ASSERT_EQ(system("cp b_tests_normal.S17FS c_tests.S17FS"), 0);
    S17FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
    int fd_array[256] = {-1};
    // OPEN_FILE 1
...
//*/
/*
   int fs_get_dir(const S17FS_t *const fs, const char *const fname, dir_rec_t *const records)
    ...
    vector<const char *> fnames{
        "/file", "/folder", "/folder/with_file", "/folder/with_folder", "/DOESNOTEXIST", "/file/BAD_REQUEST",
            ...
            "/folder/withwaytoolongfilenamethattakesupmorespacethanitshould and yet was not enough so I had to add "
                "more/bad_req",
            "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
    const char *test_fname = "f_tests.S17FS";
    ASSERT_EQ(system("cp c_tests.S17FS f_tests.S17FS"), 0);
    S17FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
    // FS_GET_DIR 1
...
//*/
/*
   ssize_t fs_write(S17FS *fs, int fd, const void *src, size_t nbyte);
...
TEST(d_tests, write_file_simple) {
    ...
    const char *test_fname = "d_tests_normal.S17FS";
    S17FS *fs = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
...
TEST(d_tests, write_file_fill) {
    ...
    vector<const char *> fnames{"/file_a", "/file_b", "/file_c", "/file_d"};
    const char *test_fname = "d_tests_full.S17FS";
    S17FS *fs = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
    uint8_t large_eight_five_b_seven[1024 * 3];
    memset(large_eight_five_b_seven, 0x88, 888);
...
//*/
/*
   int fs_remove(S17FS *fs, const char *path);
   1. Normal, file at root
...
TEST(e_tests, remove_file) {
    ...
    const char *(test_fname[2]) = {"e_tests_a.S17FS", "e_tests_b.S17FS"};
    ASSERT_EQ(system("cp d_tests_full.S17FS e_tests_a.S17FS"), 0);
    ASSERT_EQ(system("cp c_tests.S17FS e_tests_b.S17FS"), 0);
    S17FS *fs = fs_mount(test_fname[1]);
    ASSERT_NE(fs, nullptr);
    // FS_REMOVE 10
...
/*
   off_t fs_seek(S17FS *fs, int fd, off_t offset, seek_t whence)
...
TEST(g_tests, seek) {
    vector<const char *> fnames{"/file_a", "/file_b", "/file_c", "/file_d"};
    const char *test_fname = "g_tests.S17FS";
    ASSERT_EQ(system("cp d_tests_full.S17FS g_tests.S17FS"), 0);
    S17FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
...
//*/
/*
   ssize_t fs_read(S17FS *fs, int fd, void *dst, size_t nbyte);
...
int fs_move(S17FS *fs, const char *src, const char *dst);
...
/*TEST(i_tests, move) {
  ...
  "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
  const char *test_fname = "g_tests.S17FS";
  ASSERT_EQ(system("cp c_tests.S17FS g_tests.S17FS"), 0);
  S17FS *fs = fs_mount(test_fname);
  ASSERT_NE(fs, nullptr);
  int fd = fs_open(fs, fnames[0]);
// FS_MOVE 1
...
/*
   int fs_link(S17FS *fs, const char *src, const char *dst);
